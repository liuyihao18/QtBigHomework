...
public:
    ...
    void updateAll(Player                   *player,
                   ...
                   const QSet<int>         & pressedKeys); // 更新场景中的东西
    void updatePlayer(Player                   *player,
                      QSet<FlyingProp *>      & flyingProps,
                      const CollisionInspector& ci,
                      const QSet<int>         & pressedKeys);  // 更新玩家的位置
    void updateMoveThings(Player                   *player,
                          const QSet<MoveThing *> & movethings,
                          const CollisionInspector& ci);       // 更新其他移动物体的位置
    void updateLaunchers(const QSet<Launcher *>& launchers,
                         QSet<FlyingProp *>    & flyingProps); // 更新发射者的发射状态
    void updateFlyingProps(QSet<FlyingProp *>      & flyingProps,
                           const CollisionInspector& ci);      // 更新飞行物的位置
