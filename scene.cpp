...
void Scene::paintEvent(QPaintEvent *)
{
    ...
    case Loading:
        // 绘制加载图像
        p.drawPixmap(QRect(0, 0, width(), height()),
                     QPixmap::fromImage(loaderImage.getImage()));
        break;
    ...
    case Editing:
        ...
        for (auto iter = terrains.begin(); iter != terrains.end(); ++iter) {
            if ((*iter)->isShow()) {
                p.drawPixmap((*iter)->getRect(),
                             QPixmap::fromImage((*iter)->getImage()));
            }
        }
        ...
        if (player && player->isShow()) {
            p.drawPixmap(player->getRect(),
                         QPixmap::fromImage(player->getImage()));
        }
        ...
        if (gameState == Gaming) {
            ...
            if (player) {
                // 绘制人物生命
                p.drawText(QRect(200, 35, 200, 50),
                           QString::number(player->getHP()));

                // 绘制游戏分数
                p.drawText(QRect(420, 35, 200, 50),
            ...
            // 绘制游戏时间
            p.drawText(QRect(width() - 175, 35, 200, 50),
                       QTime(0,
                             0).addMSecs(clock() - gameTime).toString("mm:ss"));
        }
        break;

    case Success:
        p.drawPixmap(QRect(0, 0, width(), height()),
                     QPixmap::fromImage(successImage));
        break;

    case GameOver:
        p.drawPixmap(QRect(0, 0, width(), height()),
                     QPixmap::fromImage(gameOverImage));
        break;

    case Rank:
        // 绘制文字
        p.drawPixmap(QRect(0, 0, width(), height()),
                     QPixmap::fromImage(rankImage));
        p.scale(widthratio, heightratio);
        font.setPointSize(20);
        p.setFont(font);

        for (int i = 0; i < (10 > rankinfos.size() ? rankinfos.size() : 10);
             i++) {
            p.drawText(QRect(550, 245.2 + 58.2 * i, 200, 58), rankinfos[i].date);
            p.drawText(QRect(777, 245.2 + 58.2 * i, 150, 58), rankinfos[i].time);
            p.drawText(QRect(1030, 245.2 + 58.2 * i, 150, 58),
                       rankinfos[i].point);
            p.drawText(QRect(1250, 245.2 + 58.2 * i, 150, 58),
                       rankinfos[i].passTime);
        }
        break;
...
void Scene::mouseMoveEvent(QMouseEvent *event)
{
    ...
    case Editing:
        ...
        if (temp) {
            temp->moveRect(event->pos().x() - temp->width() * 0.5,
                           event->pos().y() - temp->height() * 0.5);
        }
        break;
...
void Scene::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    case Loading:
        ...
        if (event->button() == Qt::LeftButton) {
            if (QRect(888 * widthratio, 510 * heightratio, 230 * widthratio,
                      40 * heightratio).contains(event->pos())) {
                emit chooseSceneFile();
            }

            if (QRect(888 * widthratio, 580 * heightratio, 230 * widthratio,
                      40 * heightratio).contains(event->pos())) {
                emit newSceneFile();
            }

            if (QRect(888 * widthratio, 650 * heightratio, 230 * widthratio,
                      40 * heightratio).contains(event->pos())) {
                gameState = Rank;
                readRankFile();
            }
    ...
    case GameOver:
        ...
        if (event->button() == Qt::LeftButton) {
            if (QRect(455 * widthratio, 605 * heightratio, 265 * widthratio,
                      110 * heightratio).contains(event->pos())) {
                gameState = Gaming;
                gameRestart();
            }

            if (QRect(820 * widthratio, 605 * heightratio, 265 * widthratio,
                      110 * heightratio).contains(event->pos())) {
                emit chooseSceneFile();
            }

            if (QRect(1180 * widthratio, 605 * heightratio, 265 * widthratio,
                      110 * heightratio).contains(event->pos())) {
                emit returnHome();
            }
        }
    ...
    case Rank:
        ...
        if (event->button() == Qt::LeftButton) {
            if (QRect(870 * widthratio, 910 * heightratio, 165 * widthratio,
                      45 * heightratio).contains(event->pos())) {
                emit returnHome();
            }
        }
        break;
...
void Scene::addSceneWidget(int x, int y)
{
    ...
    // 放置精度为地图单位的一半
    temp->moveRect(temp->x() / placeAcc * placeAcc,
                   static_cast<int>((temp->y() + temp->width() * 0.15)) / placeAcc *
                   placeAcc);

    // 如果这个释放的位置没有东西，则加入，并且再次新建，保证创建的连续性
    if (ci.canAddInScene(temp->getRect())) {
        // qDebug()<<"("<<x/map_unit<<",
        // "<<y/map_unit<<")插入"<<temp->metaObject()->className();
...
void Scene::moveSceneWidget(int x, int y)
{
    ...
    if (object) {
        ...
        if (temp) {
            if ((mapFromGlobal(cursor().pos()).x() > 0) &&
                (mapFromGlobal(cursor().pos()).y() > 0)) {
                isShowChooseWidget = true;
                temp->moveRect(mapFromGlobal(
                                   cursor().pos()).x() - temp->width() * 0.5,
                               mapFromGlobal(
                                   cursor().pos()).y() - temp->height() * 0.5);
            }
        }
        isMovingThing = true;
    }
...
void Scene::readRankFile()
{
    ...
    if (file.open(QIODevice::ReadOnly)) {
        ...
        while (!in.atEnd()) {
            ...
            if (!date.isEmpty() && !time.isEmpty() && !point.isEmpty() &&
                !passTime.isEmpty()) {
...
void Scene::writeRankFile()
{
    ...
    if (player) {
        out << QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss") <<
            " " << player->getPoints() << " " << QTime(0, 0).addMSecs(
            clock() - gameTime).toString("hh:mm:ss") << endl;
    }
    file.close();
}
...
void Scene::saveScene(const QString& scenePath)
{
    ...
    // 保存地图所有组件信息
    for (auto iter = allWidgets.begin(); iter != allWidgets.end(); ++iter) {
        out << (*iter)->metaObject()->className() << " " << (*iter)->originX() <<
            " " << (*iter)->originY()
...
Scene::Scene(QWidget *parent) : QWidget(parent), gameState(Loading),
    ...
    fps(17), loading(true),
    featherCursor(QPixmap::fromImage(QImage(":/cursor/cursor/cursor.png")).scaled(
                      QSize(32, 32), Qt::KeepAspectRatio), 0, 31),
    ...
    ci(SceneInfo(m_width, m_height, &player, &goal, &allWidgets, &terrains,
                 &traps, &monsters, &buffs, &values, &flyingProps)), updater(fps,
                                                                             this),
