...
struct SceneInfo {
    ...
    SceneInfo() = default;
    SceneInfo(int                 m_width,
              int                 m_height,
              Player            **player,
              Goal              **goal,
              QSet<BaseObject *> *allWidgets,
        ...
        : m_width(m_width), m_height(m_height), player(player), goal(goal),
        allWidgets(allWidgets), terrains(terrains), traps(traps),
        monsters(monsters), buffs(buffs), values(values),
        flyingProps(flyingProps) {}
};
