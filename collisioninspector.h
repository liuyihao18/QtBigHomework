...
public:
    ...
    CollisionInspector(const SceneInfo& sceneinfo,
    ...
    BaseObject* getWidgetFromPos(int x,
                                 int y) const;                         // 返回指定位置的组件

protected:

    SceneInfo sceneinfo;
    int distance(const QRect& rect1,
                 const QRect& rect2) const; // 返回两个矩形之间的位置
